...
class Command(BaseCommand):
    ...
    #         caller.get_puppet()
    #     except:
    #         prompt = "{ride}".format(ride='R > ' if self.caller.ndb.is_riding else 'f > ')
...
class MuxCommand(BaseMuxCommand):
    ...
    def at_post_cmd(self):
        ...
        try:
            # caller.msg(f'|b>>Start Try')# |Y{type(self)})
            caller.is_typeclass('typeclasses.characters.Character')
            prompt = caller.prompt
        except:
            pass
            # caller.msg(f"|r>>Exception")# |Y{type(self)}")
        # caller.msg(f"|g>>End of post_cmd")# |Y{type(self)}")
        # if self.key == 'connect':
        #     prompt = "*. > "
        caller.msg(prompt=prompt)
